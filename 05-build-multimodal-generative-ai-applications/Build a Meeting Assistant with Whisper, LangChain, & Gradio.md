...
In this task, we use `ibm/granite-4-h-small` model as an LLM instance. You can try out other models available on [IBM watsonx.ai](https://www.ibm.com/products/watsonx-ai/foundation-models?utm_source=skills_network&utm_content=in_lab_content_link&utm_id=Lab-meeting-assistant-lab-v1_1734458732).
...
3.  **Setting model and project IDs**:
    ...
    -   `model_id`: Specifies the model being used, in this case, `ibm/granite-4-h-small`, an 8-billion parameter model designed for instructional tasks.
...
Upon running the script, you should see the generated text in your terminal, as shown in the following image. You see how watsonx Granite 4.0 provides a good answer. Note that the response you get might differ.
...
credentials = Credentials(
    url="<a href="https://us-south.ml.cloud.ibm.com/?utm_source=skills_network&utm_content=in_lab_content_link&utm_id=Lab-meeting-assistant-lab-v2-multimodalCourse_1774604104">https://us-south.ml.cloud.ibm.com</a>",
    # api_key="&lt;YOUR_API_KEY&gt;"  # Normally you'd put an API key here 
                                # but in this CloudIDE, we've taken care of that for you
)
client = APIClient(credentials)
model_id = "ibm/granite-4-h-small"</p><p>parameters = {
    GenParams.DECODING_METHOD: "sample",
    GenParams.MAX_NEW_TOKENS: 512,
    GenParams.MIN_NEW_TOKENS: 1,
    GenParams.TEMPERATURE: 0.5,
...
In our application, we used `ibm/granite-4-h-small` as the primary LLM to generate meeting summaries and task lists. Now, try replacing it with `meta-llama/llama-4-maverick-17b-128e-instruct-fp8` and compare their performance to see how the outputs differ!
